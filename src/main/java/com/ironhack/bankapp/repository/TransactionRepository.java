...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    List<Transaction> findByOriginIdOrderByTransactionDateTimeDesc(Long originId);

    @Query(value = "SELECT MAX(t.sum) FROM (SELECT DATE(transaction_date_time) AS transaction_date, SUM(transaction_amount) AS sum FROM transaction WHERE origin_id = :originId GROUP BY transaction_date) AS t", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM transaction WHERE origin_id = :originId AND transaction_date_time >= (NOW() - INTERVAL 3601 SECOND)", nativeQuery = true)
